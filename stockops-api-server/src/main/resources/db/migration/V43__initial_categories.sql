...
INSERT INTO categories (name, code, parent_id, level, sort_order, active) VALUES
    ...
    ('냉동디저트/빙과', 'FROZEN_DESSERT',   (SELECT id FROM categories WHERE code = 'FROZEN'),  2, 4, true),
    ('냉동밀키트',      'FROZEN_MEAL_KIT',  (SELECT id FROM categories WHERE code = 'FROZEN'),  2, 5, true);
...
INSERT INTO categories (name, code, parent_id, level, sort_order, active) VALUES
    ...
    ('냉장반찬/즉석',   'CHILLED_READY',     (SELECT id FROM categories WHERE code = 'CHILLED'), 2, 3, true),
    ('냉장음료',        'CHILLED_BEVERAGE',  (SELECT id FROM categories WHERE code = 'CHILLED'), 2, 4, true);

-- ── 신선식품 하위
INSERT INTO categories (name, code, parent_id, level, sort_order, active) VALUES
    ('신선채소',        'FRESH_VEGETABLE',   (SELECT id FROM categories WHERE code = 'FRESH'),   2, 1, true),
    ('신선과일',        'FRESH_FRUIT',       (SELECT id FROM categories WHERE code = 'FRESH'),   2, 2, true),
    ('신선수산물',      'FRESH_SEAFOOD',     (SELECT id FROM categories WHERE code = 'FRESH'),   2, 3, true);
...
-- ── 냉동디저트/빙과 하위
INSERT INTO categories (name, code, parent_id, level, sort_order, active) VALUES
    ('아이스크림',      'FROZEN_ICE_CREAM', (SELECT id FROM categories WHERE code = 'FROZEN_DESSERT'), 3, 1, true),
    ('빙과/빙수',       'FROZEN_ICE',       (SELECT id FROM categories WHERE code = 'FROZEN_DESSERT'), 3, 2, true);

-- ── 냉동밀키트 하위
INSERT INTO categories (name, code, parent_id, level, sort_order, active) VALUES
    ('찌개/전골 밀키트', 'FROZEN_KIT_STEW',  (SELECT id FROM categories WHERE code = 'FROZEN_MEAL_KIT'), 3, 1, true),
    ('구이/볶음 밀키트', 'FROZEN_KIT_GRILL', (SELECT id FROM categories WHERE code = 'FROZEN_MEAL_KIT'), 3, 2, true);
...
-- ── 냉장반찬/즉석 하위
INSERT INTO categories (name, code, parent_id, level, sort_order, active) VALUES
    ('반찬류',          'CHILLED_SIDEDISH', (SELECT id FROM categories WHERE code = 'CHILLED_READY'), 3, 1, true),
    ('즉석조리식품',    'CHILLED_INSTANT',  (SELECT id FROM categories WHERE code = 'CHILLED_READY'), 3, 2, true);
...
-- ── 신선채소 하위
INSERT INTO categories (name, code, parent_id, level, sort_order, active) VALUES
    ('엽채류',          'FRESH_LEAFY',     (SELECT id FROM categories WHERE code = 'FRESH_VEGETABLE'), 3, 1, true),
    ('근채류',          'FRESH_ROOT',      (SELECT id FROM categories WHERE code = 'FRESH_VEGETABLE'), 3, 2, true);

-- ── 신선과일 하위
INSERT INTO categories (name, code, parent_id, level, sort_order, active) VALUES
    ('국산과일',        'FRESH_FRUIT_KOR', (SELECT id FROM categories WHERE code = 'FRESH_FRUIT'), 3, 1, true),
    ('수입과일',        'FRESH_FRUIT_IMP', (SELECT id FROM categories WHERE code = 'FRESH_FRUIT'), 3, 2, true);

-- ── 신선수산물 하위
INSERT INTO categories (name, code, parent_id, level, sort_order, active) VALUES
    ('활어/선어',       'FRESH_FISH',         (SELECT id FROM categories WHERE code = 'FRESH_SEAFOOD'), 3, 1, true),
    ('갑각류',          'FRESH_CRUSTACEAN',   (SELECT id FROM categories WHERE code = 'FRESH_SEAFOOD'), 3, 2, true);
