...
public class InventoryService {
    ...
    /**
     * Increases inventory for a product-Warehouse-lot combination.
     *
    ...
    public Inventory increaseStock(final Long productId,
                                   ...
                                   final Long userId) {
        validateRequiredIdentifiers(productId, warehouseId, lotId);
        validateReferenceType(referenceType);
        validateQuantity(quantity);
        validateLotOwnership(productId, lotId);

        final Inventory inventory = inventoryRepository.findForUpdate(productId, warehouseId, lotId)
                .orElseGet(() -> createNewInventory(productId, warehouseId, lotId));
        ...
        recordTransaction(productId, warehouseId, lotId, "INBOUND", quantity, beforeQuantity, saved.getQuantity(),
                referenceId, userId);
        return saved;
    }

    /**
     * Decreases inventory for a product-Warehouse-lot combination.
     *
     * @param productId product id
     * @param warehouseId Warehouse id
     * @param lotId lot id
    ...
    public Inventory decreaseStock(final Long productId,
        ...
        inventory.setQuantity(beforeQuantity - quantity);
        final Inventory saved = inventoryRepository.save(inventory);
        recordTransaction(productId, warehouseId, lotId, "OUTBOUND", quantity, beforeQuantity, saved.getQuantity(),
                referenceId, userId);
        return saved;
    }

    private Inventory createNewInventory(final Long productId, final Long warehouseId, final Long lotId) {
    ...
    private void validateRequiredIdentifiers(final Long productId, final Long warehouseId, final Long lotId) {
        if (productId == null) {
            throw new InvalidOperationException("Product id is required");
        }
        if (warehouseId == null) {
            throw new InvalidOperationException("Warehouse id is required");
        }
        if (lotId == null) {
