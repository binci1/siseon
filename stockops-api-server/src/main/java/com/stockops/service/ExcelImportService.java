...
public class ExcelImportService {
    ...
    private ExcelImportResult importInbounds(final Sheet sheet) {
        ...
        for (int rowIndex = DATA_START_ROW_INDEX; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
            ...
            try {
                ...
                final String warehouseCode = requireText(getTrimmedString(row, 7, dataFormatter), "warehouseCode");
                ...
                final Warehouse Warehouse = WarehouseRepository.findByCode(warehouseCode)
                        .orElseThrow(() -> new InvalidOperationException("Warehouse not found: " + warehouseCode));
                scopeGuard.assertWarehouseAccess(Warehouse.getId());
    ...
    private void createProductsTemplate(final Workbook workbook) {
        ...
        writeExampleRow(sheet, 1, "8801234567890", "Example Item", "350ml Example", "Drink", "EA", "TRUE", "2300", "20");
    ...
    private void createInboundsTemplate(final Workbook workbook) {
        ...
        writeHeaderRow(workbook, sheet, "inboundReference", "inboundDate", "supplier", "productBarcode", "lotNumber", "expiryDate", "quantity", "warehouseCode");
        writeExampleRow(sheet, 1, "INB-APR-001", "2026-04-13", "??", "8801234567890", "LOT-20260413-01", "2026-09-30", "48", "RECEIVING-01");
        writeInstructionsSheet(workbook, "Inbounds Import", List.of(
                ...
                "productBarcode, lotNumber, quantity, and warehouseCode are required per row.",
    ...
    private void createPurchaseOrdersTemplate(final Workbook workbook) {
        ...
        writeExampleRow(sheet, 1, "PO-APR-001", "GANGNAM", "GN-01", "Example", "SUP-001", "8801234567890", "120", "2100", "Example");
    ...
    public ExcelImportService(final ProductService productService, final InboundService inboundService, final PurchaseOrderService purchaseOrderService, final ProductRepository productRepository, final WarehouseRepository WarehouseRepository, final CenterRepository centerRepository, final WarehouseRepository warehouseRepository, final PurchaseOrderRepository purchaseOrderRepository, final PurchaseOrderItemRepository purchaseOrderItemRepository, final CurrentUserProvider currentUserProvider, final ScopeGuard scopeGuard, final Validator validator) {
