...
public class PurchaseOrderService {
    ...
    public PurchaseOrder receiveShipment(Long poId, Long shipmentId, Long userId) {
        ...
        Warehouse targetWarehouse = resolveTargetWarehouse(po);
        ...
        if (!shipmentItems.isEmpty()) {
            for (PurchaseOrderShipmentItem shipmentItem : shipmentItems) {
                ...
                createInboundItemAndUpdateInventory(inbound, product, quantity, targetWarehouse, userId);
    ...
    private List<PurchaseOrder> filterScopedPurchaseOrders(final List<PurchaseOrder> purchaseOrders) {
        return scopeGuard.filterByCenterWarehouseScope(
                ...
                purchaseOrder -> purchaseOrder.getTargetWarehouseId() == null ? null : purchaseOrder.getTargetWarehouseId().getId());
    }
    ...
    private void assertPurchaseOrderAccess(final PurchaseOrder purchaseOrder) {
        scopeGuard.assertCenterWarehouseAccess(
                ...
                purchaseOrder.getTargetWarehouseId() == null ? null : purchaseOrder.getTargetWarehouseId().getId());
    }
    ...
    private Warehouse resolveTargetWarehouse(final PurchaseOrder po) {
        if (po.getTargetWarehouseId() != null) {
            return po.getTargetWarehouseId();
        }

        List<Warehouse> receivingWarehouses = WarehouseRepository.findByType("RECEIVING");
        if (!receivingWarehouses.isEmpty()) {
            return receivingWarehouses.get(0);
        }
    ...
    private void createInboundItemAndUpdateInventory(final Inbound inbound,
        ...
        inboundItem.setWarehouseId(Warehouse.getId());
