...
public class InboundService {
     ...
     * @throws ResourceNotFoundException when inbound, product, or Warehouse does not exist
    ...
    public InboundItemDTO addItem(final Long inboundId, final AddInboundItemRequest request) {
        ...
        final Warehouse Warehouse = findWarehouseById(request.warehouseId());
        assertWarehouseWarehouseNotClosed(Warehouse.getId());
        ...
        return toItemDTO(saved, product, Warehouse);
    }
    ...
    public InboundDTO confirmInbound(final Long inboundId, final Long userId) {
        ...
        for (InboundItem item : items) {
            ...
            findWarehouseById(item.getWarehouseId());
            assertWarehouseWarehouseNotClosed(item.getWarehouseId());
    ...
    private Warehouse findWarehouseById(final Long warehouseId) {
    ...
    private void assertWarehouseWarehouseNotClosed(final Long warehouseId) {
        final Warehouse Warehouse = findWarehouseById(warehouseId);
        if (Warehouse != null && Warehouse.getStatus() == WarehouseStatus.CLOSED) {
            throw new InvalidOperationException(
                    "Inbound not allowed to closed warehouse: " + Warehouse.getName());
        }
    }
    ...
    private InboundItemDTO toItemDTO(final InboundItem item) {
        ...
        final Warehouse Warehouse = findWarehouseById(item.getWarehouseId());
        return toItemDTO(item, product, Warehouse);
    }

    private InboundItemDTO toItemDTO(final InboundItem item, final Product product, final Warehouse Warehouse) {
