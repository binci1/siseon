...
public class OutboundService {
     ...
     * The concrete FEFO lot alwarehouse is deferred until confirmation.
     *
    ...
    public OutboundDTO confirmOutbound(final Long outboundId, final Long userId) {
        ...
        for (final OutboundItem item : items) {
            final List<OutboundLotAllocation> alwarehouses = allocateLotsByFefo(item, outboundId, userId);
            applyAlwarehouses(item, outboundId, alwarehouses);
        }
     ...
     * @param outboundId outbound id
     * @param userId operator user id
     * @return concrete lot alwarehouses used to satisfy the request
    ...
    private List<OutboundLotAllocation> allocateLotsByFefo(final OutboundItem item,
        ...
        final List<OutboundLotAllocation> alwarehouses = new ArrayList<>();
        ...
        for (final Lot lot : lots) {
            ...
            final List<Inventory> candidateInventories = inventoryRepository.findByProductIdAndLotId(item.getProductId(), lot.getId())
                    ...
                    .sorted(Comparator.comparing(Inventory::getWarehouseId))
                    .toList();
            final List<Inventory> inventories = scopeGuard.filterByWarehouseScope(candidateInventories, Inventory::getWarehouseId);
            ...
            for (final Inventory inventory : inventories) {
                ...
                assertWarehouseWarehouseNotClosed(inventory.getWarehouseId());
                ...
                alwarehouses.add(new OutboundLotAllocation(lot.getId(), deductedQuantity));
        ...
        if (remainingQuantity > 0) {
            if (totalEligibleQuantity > totalAccessibleQuantity) {
                throw new ForbiddenException("Access denied for outbound inventory alwarehouse");
            }
    ...
    /**
     * Persists FEFO alwarehouses by updating the original item with the primary lot and creating split items for additional lots.
     *
     * @param item original draft item
     * @param outboundId outbound id
     * @param alwarehouses FEFO lot alwarehouses
     */
    private void applyAlwarehouses(final OutboundItem item,
                                  final Long outboundId,
                                  final List<OutboundLotAllocation> alwarehouses) {
        if (alwarehouses.isEmpty()) {
            throw new InvalidOperationException("Cannot confirm outbound without FEFO alwarehouses");
        }

        final OutboundLotAllocation primaryAlwarehouse = alwarehouses.getFirst();
        item.setLotId(primaryAlwarehouse.lotId());
        item.setQuantity(primaryAlwarehouse.quantity());
        outboundItemRepository.save(item);

        for (int i = 1; i < alwarehouses.size(); i++) {
            final OutboundLotAllocation alwarehouse = alwarehouses.get(i);
            outboundItemRepository.save(createAllocatedItem(
                    outboundId,
                    item.getProductId(),
                    alwarehouse.lotId(),
                    alwarehouse.quantity()));
        }
    }
    ...
    private boolean canAccessOutbound(final Outbound outbound, final Long currentUserId) {
        final List<InventoryTransaction> alwarehouses = inventoryTransactionRepository
        ...
        return scopeGuard.filterByWarehouseScope(alwarehouses, InventoryTransaction::getWarehouseId).size() == alwarehouses.size();
    }
    ...
    private void assertWarehouseWarehouseNotClosed(final Long warehouseId) {
        ...
        if (Warehouse != null && Warehouse.getStatus() == WarehouseStatus.CLOSED) {
            throw new InvalidOperationException(
                    "Outbound not allowed from closed warehouse: " + Warehouse.getName());
        }
    }
