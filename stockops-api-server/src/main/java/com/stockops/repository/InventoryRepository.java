...
public interface InventoryRepository extends JpaRepository<Inventory, Long> {
     ...
     */
    Optional<Inventory> findByProductIdAndWarehouseIdAndLotId(Long productId, Long warehouseId, Long lotId);
    ...
    /**
     * Finds inventory rows for a product and lot across all Warehouses.
     *
     * @param productId product id
     * @param lotId lot id
    ...
    /**
     * Sums inventory quantity for a product across all Warehouses.
     *
     * @param productId product id
    ...
    /**
     * Sums inventory quantity grouped by product for given Warehouses.
     *
     * @param warehouseIds Warehouse ids
     * @return list of [productId, totalQuantity] pairs
     */
    @Query("SELECT i.productId, COALESCE(SUM(COALESCE(i.quantity, 0)), 0) FROM Inventory i WHERE i.warehouseId IN :warehouseIds GROUP BY i.productId")
    List<Object[]> sumQuantityByWarehouseIdsIn(@Param("warehouseIds") List<Long> warehouseIds);
