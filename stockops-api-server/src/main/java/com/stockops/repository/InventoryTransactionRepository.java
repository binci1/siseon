...
public interface InventoryTransactionRepository extends JpaRepository<InventoryTransaction, Long> {
    ...
    /**
     * Returns transaction history for a Warehouse.
     *
    ...
    /**
     * Returns outbound transactions for a set of Warehouses within a date range.
     * Used by ABC/XYZ classification to compute product demand within a center's warehouses.
     *
     * @param warehouseIds Warehouse identifiers (warehouses in the center)
     * @param start inclusive range start
     * @param end inclusive range end
     * @return matching outbound transactions
     */
    @Query("SELECT t FROM InventoryTransaction t WHERE t.warehouseId IN :warehouseIds AND t.type = 'OUTBOUND' AND t.createdAt BETWEEN :start AND :end")
    List<InventoryTransaction> findOutboundByWarehouseIdsAndCreatedAtBetween(
    ...
    /**
     * Sums outbound transaction quantities grouped by product for Warehouses in a date range.
     *
     ...
     */
    @Query("SELECT t.productId, COALESCE(SUM(t.quantity), 0) FROM InventoryTransaction t WHERE t.warehouseId IN :warehouseIds AND t.type = :type AND t.createdAt BETWEEN :start AND :end GROUP BY t.productId")
    List<Object[]> sumQuantityByWarehouseIdsAndTypeAndCreatedAtBetween(
            @Param("warehouseIds") List<Long> warehouseIds,
            @Param("type") String type,
            @Param("start") Instant start,
            @Param("end") Instant end);
