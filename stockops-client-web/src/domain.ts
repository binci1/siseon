...
export function canManagePurchaseOrder(user: AuthenticatedUser | null, order: PurchaseOrder): boolean {
  ...
  // Backend may return requestedBy as a User object or as a plain number ID
  const requestedById =
    typeof order.requestedBy === 'object' && order.requestedBy !== null
      ? (order.requestedBy as { id: number }).id
      : order.requestedBy
  return user.id === requestedById || normalizeRole(user.role) === 'STORE_MANAGER'
}
