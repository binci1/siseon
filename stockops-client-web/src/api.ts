...
export async function cancelPurchaseOrder(orderId: number, reason?: string): Promise<PurchaseOrder> {
  const params = new URLSearchParams({ reason: reason ?? '클라이언트 취소' })
  return apiRequest<PurchaseOrder>(`/v1/purchase-orders/${orderId}/cancel?${params.toString()}`, { method: 'POST' })
}
