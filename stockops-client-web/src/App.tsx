...
/** Ensure we always work with arrays even if the API returns null/undefined/object */
...
/**
 * Simple error boundary wrapper to catch render errors in child components.
 * Prevents a crash in OrdersPage from causing a full white screen.
 */
    ...
    if (this.state.hasError) {
      return (
        <div className="center-screen">
          <p className="error-text">발주 화면을 불러오는 중 오류가 발생했습니다.</p>
          <small>{this.state.error?.message}</small>
          <button className="ghost-button" onClick={() => this.setState({ hasError: false })}>
            다시 시도
          </button>
        </div>
      )
...
function InventoryPage() {
  ...
  const rows = useMemo(
    () => filterInventory(toArray(inventory.data), { query, centerId, warehouseId, lowStockOnly }),
    ...
    <section className="panel">
      <div className="toolbar">
        ...
        <select value={centerId} onChange={(event) => setCenterId(event.target.value)}>
          ...
          {toArray(centers.data).map((center) => (
        ...
        <select value={warehouseId} onChange={(event) => setWarehouseId(event.target.value)}>
          ...
          {toArray(warehouses.data).map((warehouse) => (
...
function OrdersPage({ user }: { user: AuthenticatedUser }) {
  ...
  const displayedOrder = useMemo(() => {
    ...
    return toArray(orders.data).find((order) => order.id === selectedOrder.id) ?? selectedOrder
  }, [orders.data, selectedOrder])

  const canManageDisplayedOrder = Boolean(
    displayedOrder && canManagePurchaseOrder(user, displayedOrder),
  )
  ...
  const cancelMutation = useMutation({
    mutationFn: (id: number) => cancelPurchaseOrder(id, '클라이언트 요청으로 취소'),
    onSuccess: (updatedOrder) => {
      ...
      <section className="panel">
        ...
        >
          ...
          <SelectField label="상품" value={draft.productId} error={errors.productId} onChange={(value) => setDraft({ ...draft, productId: value })}>
            ...
            {toArray(products.data).map((product) => (
      ...
      <section className="panel">
        ...
        {orders.isLoading ? (
          ...
          <div className="order-list">
            ...
            {toArray(orders.data).length === 0 && <EmptyState title="발주 내역이 없습니다." />}
          </div>
        )}
        {displayedOrder && (
          <section className="panel">
            ...
            <h4>품목</h4>
            <div className="order-list">
              {toArray(displayedOrder.items).map((item) => (
              ...
              {toArray(displayedOrder.items).length === 0 && <EmptyState title="발주 품목이 없습니다." />}
            </div>
...
function ChatSession() {
  ...
  const messages = [...toArray(history.data), ...localMessages]
