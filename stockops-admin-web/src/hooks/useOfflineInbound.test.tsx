...
describe('useOfflineInbound', () => {
  ...
  it('saves inbound data offline and updates pending count', async () => {
    vi.mocked(offlineStorage.saveInbound).mockResolvedValue({ id: 1, productBarcode: '123', quantity: 10, lotNumber: 'LOT1',   centerId: 1, createdAt: '2024-01-01T00:00:00Z', synced: 0 })
    vi.mocked(offlineStorage.countPending).mockResolvedValue(1)

    const { result } = renderHook(() => useOfflineInbound(), { wrapper: createWrapper() })

    await act(async () => {
      await result.current.saveOffline({ productBarcode: '123', quantity: 10, lotNumber: 'LOT1',   centerId: 1 })
    })

    expect(offlineStorage.saveInbound).toHaveBeenCalledWith({ productBarcode: '123', quantity: 10, lotNumber: 'LOT1',   centerId: 1 })
    expect(result.current.pendingCount).toBe(1)
  })

  it('syncs pending records and clears them on success', async () => {
    const pending: PendingInbound[] = [
      { id: 1, productBarcode: '123', quantity: 10, lotNumber: 'LOT1',   centerId: 1, createdAt: '2024-01-01T00:00:00Z', synced: 0 },
    ]
